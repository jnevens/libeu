...
eu_object_t *object_get_child(eu_object_t *obj, const char *name)
{
	...
	{
		eu_object_t *child = list_node_data(node);
...
{
	obj->child_counter = 0;
	eu_list_node_t *node = list_first(obj->children);
...
eu_object_t *object_get_last_child(eu_object_t *obj)
{
	...
	eu_list_node_t *node = list_last(obj->children);
	return (node) ? list_node_data(node) : NULL;
}
...
eu_object_t *object_get_next_child(eu_object_t *obj)
{
	...
	list_for_each(node, obj->children)
	{
		if (obj->child_counter == count)
			return (eu_object_t *) list_node_data(node);
		count++;
	}
	return NULL;
}
...
eu_parameter_t *object_get_parameter(eu_object_t *obj, const char *name)
{
	...
	if (obj && name) {
		...
		{
			eu_parameter_t *parameter = list_node_data(node);
...
{
	bool rv = false;
	eu_parameter_t *param = object_get_parameter(obj, name);
	if (param) {
...
void object_print(eu_object_t *obj)
{
	...
	list_for_each(node, obj->parameters) {
		eu_parameter_t *child = list_node_data(node);
